...
const handleUpload = async () => {
  ...
  else trendData = trendData.slice(-30); 
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["Daily", "Weekly", "Monthly"].map((t) => (
                <button 
                  ...
                  style={{ background: trendMode === t ? colors.blue : C.panelAlt, color: trendMode === t ? "#fff" : C.textDim, border: "none", padding: "4px 12px", borderRadius: 16, fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>
                  {t}
                </button>
              ))}
            </div>
              ...
              return (
                <div key={r.id || i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px", background: C.bg, borderRadius: 8, border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: isInbound ? colors.blue+"22" : colors.purple+"22", color: isInbound ? colors.blue : colors.purple, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 13 }}>{initials}</div>
                    <div><div style={{ fontWeight: 600, fontSize: 13, color: C.text }}>{r.staff}</div><div style={{ fontSize: 11, color: C.textDim }}>{r.dateStr}</div></div>
                  </div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: isInbound ? colors.blue : colors.purple, background: isInbound ? colors.blue+"11" : colors.purple+"11", padding: "4px 10px", borderRadius: 12, border: `1px solid ${isInbound ? colors.blue+"40" : colors.purple+"40"}` }}>{r.type || "Inbound"}</div>
                </div>
              );
            })}
            ...
            {staffData.map((staff, i) => {
              ...
              return (
                <div key={staff.name} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 24, height: 24, borderRadius: "50%", background: rankColors[i] || C.textDim, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: C.panelAlt, color: C.text, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{initials}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 4 }}>{staff.name}</div>
                    <div style={{ height: 6, background: C.panelAlt, borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: `${Math.min((staff.score / 100) * 100, 100)}%`, background: rankColors[i] || C.textDim, borderRadius: 3 }} /></div>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: rankColors[i] || C.textDim, width: 24, textAlign: "right" }}>{staff.score}</div>
                </div>
              )
            })}
