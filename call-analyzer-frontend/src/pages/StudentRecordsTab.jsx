...
// Notice we are correctly using the 'C' prop here now!
export default function StudentRecordsTab({ dbRecords, C }) {
  const [search, setSearch] = useState("");
  const [expandedRow, setExpandedRow] = useState(null);

  // ─── BRAND COLORS (These stay colorful regardless of Dark/Light mode) ───
  const brandColors = {
    blue: "#0284C7", green: "#10B981", orange: "#F59E0B", red: "#EF4444", purple: "#8B5CF6"
  };

  // ─── HELPER: FORMAT COURSE (Stops the table from stretching) ───
  // This takes "BBA (General...)" and just returns "BBA"
  const formatCourse = (courseStr) => {
    if (!courseStr) return "N/A";
  ...
  const StatCard = ({ title, value, sub, icon, color }) => (
    <div style={{ background: C.panel, borderRadius: 12, padding: "20px", flex: 1, minWidth: 180, border: `1px solid ${C.border}`, boxShadow: C.shadow }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        <div style={{ fontSize: 24, background: color+"15", width: 40, height: 40, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: color }}>
          {icon}
        </div>
  ...
  const Pill = ({ text }) => {
    ...
    // Dynamic pill coloring based on text
    ...
    return (
      <span style={{ color: color, background: bg, border: `1px solid ${color}40`, padding: "4px 10px", borderRadius: 12, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>
        {text || "Pending"}
      </span>
    );
  };
    ...
    <div style={{ animation: "fadeIn 0.5s" }}>
      ...
      <div style={{ display: "flex", gap: 12, background: C.panel, padding: "12px 16px", borderRadius: 12, border: `1px solid ${C.border}`, marginBottom: 20, alignItems: "center", boxShadow: C.shadow }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input 
            type="text" 
            placeholder="Search student, counselor, city..." 
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            style={{ width: "100%", padding: "10px 10px 10px 36px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.inputBg || C.bg, color: C.text, fontSize: 13, outline: "none" }}
          />
        </div>
        
        {/* Dropdowns use C.panel for correct dark/light mode background */}
        <select style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.panelAlt, fontSize: 13, color: C.text, outline: "none", cursor: "pointer" }}>
        ...
        <div style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.panelAlt, fontSize: 13, color: C.textDim }}>
          📅 Last 30 Days
        </div>
        
        <div style={{ fontSize: 13, color: C.textDim, padding: "0 12px" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", whiteSpace: "nowrap" }}>
            ...
            <tbody>
              {filteredRecords.map((r, i) => {
                ...
                const cleanCourse = formatCourse(r.course);
                ...
                return (
                  <React.Fragment key={r.id || i}>
                    ...
                    <tr 
                      ...
                      style={{ borderBottom: `1px solid ${C.border}`, color: C.text, fontSize: 13, background: isExpanded ? C.panelAlt : "transparent", cursor: "pointer", transition: "background 0.2s" }}
                      onMouseOver={e => !isExpanded && (e.currentTarget.style.background = C.panelAlt)}
                    ...
                    >
                      ...
                      <td style={{ padding: "16px 10px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 28, height: 28, borderRadius: "50%", background: brandColors.blue+"20", color: brandColors.blue, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>
                            {counselorInitials}
                          </div>
                          <span style={{ fontWeight: 600 }}>{r.staff?.split(" ")[0]}</span>
                        </div>
                      </td>
                      ...
                      <td style={{ padding: "16px 10px", color: C.textDim, fontWeight: 600 }}>
                        {cleanCourse} {/* 👈 Output the truncated course name */}
                      </td>
                      ...
                      <td style={{ padding: "16px 10px", width: 140 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          ...
                          <span style={{ fontWeight: 700, color: probColor, fontSize: 12 }}>{prob}%</span>
                        </div>
                      </td>
                      
                      <td style={{ padding: "16px 20px", textAlign: "center", color: C.textDim }}>
                        <button style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4, width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: brandColors.blue, transition: "transform 0.2s", transform: isExpanded ? "rotate(180deg)" : "rotate(0deg)" }}>
                          ▼
                        </button>
                      </td>
                    </tr>
                      ...
                      <tr style={{ background: C.panelAlt, borderBottom: `2px solid ${C.border}` }}>
                        <td colSpan="10" style={{ padding: 0 }}>
                          <div style={{ padding: "20px 40px", display: "flex", gap: 24, borderLeft: `4px solid ${brandColors.blue}` }}>
                            ...
                            <div style={{ flex: 1.5 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, color: C.textDim, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>
                                <span>💬</span> Conversation Summary
                              </div>
                              <div style={{ background: C.panel, padding: 20, borderRadius: 12, border: `1px solid ${C.border}`, color: C.text, fontSize: 14, lineHeight: 1.6, boxShadow: C.shadow }}>
                                {r.remark}
                              </div>
                            </div>
                            ...
                            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
                              <div style={{ display: "flex", gap: 12 }}>
                                <div style={{ flex: 1, background: C.panel, padding: "12px 16px", borderRadius: 8, border: `1px solid ${C.border}` }}>
                                  <div style={{ color: C.textDim, fontSize: 10, fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>Call Type</div>
                                  <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{r.type || "Inbound"}</div>
                                </div>
                                <div style={{ flex: 1, background: C.panel, padding: "12px 16px", borderRadius: 8, border: `1px solid ${C.border}` }}>
                                  <div style={{ color: C.textDim, fontSize: 10, fontWeight: 700, textTransform: "uppercase", marginBottom: 4 }}>AI Star Rating</div>
                                  <div style={{ fontSize: 14, fontWeight: 600, color: brandColors.orange, letterSpacing: 2 }}>{r.starRating}</div>
                                </div>
                              </div>
                              
                              <div style={{ background: C.panel, padding: "16px", borderRadius: 8, border: `1px solid ${C.border}` }}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                                  <div style={{ color: C.textDim, fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>Admission Probability</div>
                                  <div style={{ fontWeight: 800, color: probColor, fontSize: 12 }}>{prob}%</div>
                                </div>
              ...
              {filteredRecords.length === 0 && (
                <tr>
                  <td colSpan="10" style={{ padding: "40px", textAlign: "center", color: C.textDim, fontSize: 14 }}>
